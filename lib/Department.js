const connection = require("./Database");

class Department {
    constructor(name, id = "") {
        this.name = name;
        this.id = id;
    }

    displayDepartment = () => {
        console.log(`Department ID: ${this.id}`);
        console.log(`Department Name: ${this.name}`);
        console.log(`-----------------------------------`);
    }

    setId = id => {
        this.id = id;
    }

    async saveDepartmentToDatabase() {
        const promise = new Promise((resolve, reject) => {

            connection.query(`INSERT INTO department(department_name) VALUES(?)`, [this.name], (error, results, field) => {
                if (error) {
                    throw (error);
                }
    
                this.setId(results.insertId);
                resolve(results.insertId);
            })

        })        

        return await promise;
    }

    updateDepartmentInDatabase(){
        connection.query("UPDATE department SET ? WHERE ?", [{department_name: this.name}, {id: this.id}], (err, result) => {
            if(err){
                throw err
            }
            
        })
    }

    static async getDepartments() {
        const promise = new Promise((resolve, reject) => {
            
            connection.query(`SELECT * FROM department ORDER BY id`, (error, results, field) => {
                if (error) {
                    throw (error);
                }
                
                resolve(results.map(department => ({name: department.department_name, value: department})));

            })
        })
        
        return await promise;
    }

    static async viewDepartments(){
        console.table((await Department.getDepartments()).map(department => department.value) );
    }
}



module.exports = Department;